...
namespace Hotelreservationsystem
{
    class Hotel
    {
        ...
        public uint WeekdayRewardRate { get; set; }
        public uint WeekendRewardRate { get; set; }
    ...
    class Program
    {
        ...
        public static void choices()
        {
            ...
            Console.WriteLine("1. Add hotel name and Regular price of hotel.\n" +
                              ...
                              "8. Ability to add special rates for reward customers\n"+
                              "9. Ability to find cheapest Best rated Hotel for reward customer\n"+
                              "10. Exit the program.");
            ...
            switch (choice)
            {
                ...
                case 5:
                    ...
                    BestRated_Hotel();
                        choices();
                    break;
                    case 8:Ability_To_Add_SpecialRatesFor_RewardCustomers();
                    choices();
                        break;
                    case 9:
                    CheapestBestRatedHotelForRewardCustomer();
                        choices();
                    break;
                    case 10:
                    break;
        ...
        public static void Ability_To_Add_SpecialRatesFor_RewardCustomers()
        {
            for (int i = 0; i < hotels.Count; i++)
            {
                Console.WriteLine("Enter the WeekendRewardRate for the " + hotels[i].Name);
                hotels[i].WeekendRewardRate =Convert.ToUInt32(Console.ReadLine());
                Console.WriteLine("Enter the WeekDayRewardRate for the " + hotels[i].Name);
                hotels[i].WeekdayRewardRate = Convert.ToUInt32(Console.ReadLine());
            }
        }
        public static void CheapestBestRatedHotelForRewardCustomer()
        {
            Console.WriteLine("Enter the Customer Type , Regular or Rewards");
            string customerType = Console.ReadLine();
            if (customerType.ToLower() != "regular" && customerType.ToLower() != "rewards")
                throw new ArgumentException("Invalid customer type. Please provide 'regular' or 'rewards'.");
            Console.WriteLine("Enter the start date: ");
            DateTime startdate = DateTime.Parse(Console.ReadLine());
            Console.WriteLine("Enter the End Date");
            DateTime enddate = DateTime.Parse(Console.ReadLine());
            var availableHotels = hotels.Select(hotel =>
            {
                uint totalRate = Cheapest(customerType, startdate, enddate, hotel);
                return new { hotel.Name, hotel.Rating, totalRate };
            });
            var cheapestHotel = availableHotels.OrderBy(hotel => hotel.totalRate)
                                                .ThenByDescending(hotel => hotel.Rating)
        ...
        public static uint Cheapest(string customerType, DateTime start, DateTime end, Hotel hotel)
        {
            uint totalRate = 0;
            for (DateTime i = start; i <= end;)
            {
                if (i.DayOfWeek == DayOfWeek.Saturday || i.DayOfWeek == DayOfWeek.Sunday)
                {
                    totalRate += customerType.ToLower() == "regular" ? hotel.WeekendRegularRate : hotel.WeekendRewardRate;
                }
                else
                {
                    totalRate += customerType.ToLower() == "regular" ? hotel.WeekdayRegularRate : hotel.WeekdayRewardRate;
                }
                i = i.AddDays(1);
            }
            return totalRate;
